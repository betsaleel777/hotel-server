...
class ArticlesController extends Controller
{
    ...
    {
        $articles = DB::select(
            DB::raw("SELECT m1.id,m1.mesure,m1.deleted_at,m1.etagere,m1.contenance,m1.nom,m1.prix_unitaire,m1.disponible-IFNULL(m2.sortie,0) AS disponible,IFNULL(m2.reste,0) AS reste FROM (SELECT t1.id,t1.mesure,t1.deleted_at,t1.etagere,t1.contenance,t1.nom,t1.prix_unitaire,COALESCE(t1.entree-t2.quantite,t1.entree) AS disponible FROM (SELECT ae.id,ae.nom,ae.mesure,ae.deleted_at,ae.etagere,ae.contenance,IFNULL(SUM(ade.quantite),0) AS entree,COALESCE (NULLIF(AVG(ade.cout),0),ae.prix_vente) AS prix_unitaire FROM articles_depenses_externes ade RIGHT JOIN articles_externes ae ON ade.article_id = ae.id
...
((SELECT t1.id,t1.mesure,t1.deleted_at,t1.etagere,t1.contenance,t1.nom,sum(t1.sortiecl+t2.sortiecl) DIV t1.contenance AS sortie,100 - sum(t1.sortiecl+t2.sortiecl) MOD t1.contenance*100/t1.contenance AS reste
...
SELECT ae.id,ae.mesure,ae.deleted_at,ae.etagere,ae.contenance,ae.nom,t.sortiecl FROM articles_externes ae INNER JOIN tournees t ON ae.id=t.article_id
WHERE ae.restaurant_id = $restaurant GROUP BY ae.id) AS t1
RIGHT JOIN
